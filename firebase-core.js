...
export function avatarUrl(value) {
  ...
  return value.url || DEFAULT_AVATAR;
}

export function avatarColorFromUrl(url) {
...
export async function createProfileForUser(user, username) {
  ...
  const profile = {
    uid: user.uid,
    username: cleanName,
    usernameLower: normalize(cleanName),
    email: user.email || "",
...
export async function renderHeaderAuth() {
  const authBox = document.querySelector("[data-auth]");
  if (!authBox) return;

  const user = await authReady();
  if (!user) {
    authBox.innerHTML = `<a class="btn" href="login.html">Log in</a><a class="btn primary" href="signin.html">Sign up</a>`;
    return;
  }

  const profile = await ensureProfile(user);
  authBox.innerHTML = `
    <a class="btn primary" href="studio.html">Studio</a>
    <span class="tiny">Signed in as ${escapeHtml(profile?.username || user.email)}</span>
